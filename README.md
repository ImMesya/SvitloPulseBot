...
### Світлодіодна індикація (D2)
Скрипт використовує вбудований світлодіод (зазвичай GPIO 2) для візуального контролю:
- **Світлодіод не горить** — ESP32 намагається підключитися до Wi-Fi або виникла помилка.
- **Світлодіод горить** — Wi-Fi підключено успішно, пристрій працює і надсилає сигнали.

### Стабільність підключення
Скрипт `esp32_heartbeat.py` містить покращену логіку для сучасних роутерів (WiFi 6/7):
- Автоматичне скидання стека Wi-Fi при помилках.
- Збільшений час очікування підключення (30 с).
- Встановлення `dhcp_hostname` для кращої сумісності з роутерами.
- **Самодіагностика:** якщо виникає критична помилка «Internal State Error» (часта проблема ESP32 після м'якої перезагрузки), пристрій автоматично виконає повне перезавантаження (Hard Reset).
